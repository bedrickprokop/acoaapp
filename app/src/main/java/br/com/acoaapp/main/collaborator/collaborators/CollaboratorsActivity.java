...
import br.com.acoaapp.main.commons.ItemListListener;
import br.com.acoaapp.R;
import br.com.acoaapp.main.collaborator.collaboratordetail.CollaboratorDetailActivity;
import br.com.acoaapp.main.collaborator.maintaincollaborator.MaintainCollaboratorActivity;
...
public class CollaboratorsActivity extends AppCompatActivity implements CollaboratorsContract.View {
    ...
    protected void onSaveInstanceState(Bundle outState) {
        outState.putSerializable(MessageConstants.INTENT_KEY_COLLABORATORLIST, mAdapter.getList());
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case MessageConstants.REQUESTCODE_ADDCOLLABORATOR:
                if (resultCode == RESULT_OK) {
                    Collaborator collaborator = (Collaborator) data.getSerializableExtra(
                            MessageConstants.INTENT_KEY_COLLABORATOR);

                    String name = collaborator.getName().split(StringConstants.SPACE)[0];
            ...
            case MessageConstants.REQUESTCODE_EDITCOLLABORATOR:
                if (resultCode == RESULT_OK) {
                    boolean reloadCollaborators = data.getBooleanExtra(
                            MessageConstants.INTENT_KEY_RELOAD_COLLABORATORS, true);
    ...
    private void loadData(Bundle savedInstanceState) {
        if (null != savedInstanceState) {
            mAdapter.addData((CopyOnWriteArrayList<Collaborator>) savedInstanceState
                    .get(MessageConstants.INTENT_KEY_COLLABORATORLIST));
        } else {
            mActionListener.loadCollaboratorList(true);
        }
    }
    ...
    public void showCollaboratorDetailUi(Integer collaboratorCode) {
        ...
        intent.putExtra(MessageConstants.INTENT_KEY_COLLABORATORCODE, collaboratorCode);
        startActivityForResult(intent, MessageConstants.REQUESTCODE_EDITCOLLABORATOR);
    }

    @Override
    public void showAddCollaboratorUi() {
        Intent intent = new Intent(this, MaintainCollaboratorActivity.class);
        startActivityForResult(intent, MessageConstants.REQUESTCODE_ADDCOLLABORATOR);
    }
