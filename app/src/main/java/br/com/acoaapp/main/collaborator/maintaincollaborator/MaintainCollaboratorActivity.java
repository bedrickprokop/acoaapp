...
import br.com.acoaapp.data.entity.CollaboratorProfileEnum;
...
public class MaintainCollaboratorActivity extends AppCompatActivity implements MaintainCollaboratorContract.View {
    ...
    private void setupExtras() {
        ...
        if (null != extras) {
            isInsertMode = false;
            collaborator = (Collaborator) extras.getSerializable(MessageConstants.INTENT_KEY_COLLABORATOR);

        } else {
            isInsertMode = true;
            collaborator = new Collaborator(StringConstants.EMPTY, StringConstants.EMPTY,
                    CollaboratorProfileEnum.ADMINISTRATOR, StringConstants.EMPTY);
        }
    }
